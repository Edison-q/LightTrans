...
- **批量保存**：多选栏新增「💾 保存」——选中文件打包成 `lighttrans-时间戳.zip` 下载（服务端 `/api/zip?ids=…`，ZIP_STORED 原样打包不重复压缩、失效文件自动跳过、路径穿越已防护）；选中纯文字消息时按钮置灰；失效图片自动跳过并提示。
...
- **手机端批量保存未做**：iOS 网页无法批量写相册。候选：zip 进「文件」App（通用）或 iOS 快捷指令逐张存相册（体验最佳），待用户选。
