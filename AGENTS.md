...
> 给未来的协作者（AI 助手/贡献者）的项目速览。最近更新：2026-09-12。
...
## 里程碑 2026-09-12：时间显示 + iOS 多选修复（c96daeb）

- **时间智能显示**：fmtTime 两级判定——今天只显时间（时:分）、跨天显日期（月/日 时:分）、跨年带年份。
- **iOS 多选只发第一张修复**：`input.files` 是活引用，逐文件上传途中清空输入框会截断 FileList——sendFiles 改为先快照 FileList 数组再清空 input。
- 批量保存细节（09-11~12）：纯文字消息不参与保存（按钮置灰）；手机端多选按钮栏压缩适配窄屏。

## 里程碑 2026-09-14：长按菜单分享 + 触屏精简（未提交）

- **长按菜单加「分享」**：文件卡片 [下载 · 分享 · 删除]、图片 [查看图片 · 分享 · 删除]、文字不变。**双路实现**：① 有网页分享（https/localhost）→ `navigator.share({files})` 系统面板；② 无网页分享（http，**iOS 在纯 http 下禁用 Web Share API**，实测确认）→ 先轻量 fetch 确认文件存在（AbortController 只取响应头）再 `window.open(url,'_blank')` 开 Safari 原生预览页（自带分享/存储按钮）；失效文件 toast 不动作，取消分享静默。
- **图片卡片恢复原生长按**：`.filecard .imgbox img` 重开 `-webkit-touch-callout:default`（覆盖 .msg 的 none），长按图片本身= iOS 原生菜单（存储图像/共享/拷贝，一步存相册）；attachLongPress 对 img 目标短路，长按图片下方文字区仍弹自家菜单。图片提示文案「点图放大 · 长按存相册」。
- **触屏端隐藏卡片「下载」按钮**（`@media (hover:none)`），下载走长按菜单；电脑端保留。文件卡提示按平台「长按下载/分享」/「点右侧下载」。
- **验证**（headless Edge CDP，移动仿真+注入模拟 http）：PC 面板路径逐字节交付、失效 toast、http 新标签路径 URL 精确（含 ?k= 令牌，实测 `/api/files/tokentest.txt?k=testtoken123`）、长按 img/文字区分路由、CSS 隐藏与提示文案——20+ 断言全过。无头模式不落 window.open 标签页（环境限制），真机开标签页与原生菜单需用户 iPhone 确认。
- **实测新发现（iPhone 用户端）**：图片原生长按（存储图像/共享）实测满意 ✓。文件「分享→新标签页」在用户 iPhone 上被 Safari 拦截，报错「导览失败，因为要求是针对已启用"仅限HTTPS"的 HTTP URL」——**用户的 Safari 开着「仅限 HTTPS」设置**（疑似 iOS 26.6.1 更新引入），拦截一切新 http 页面导航（App 首页能开是因为之前手动点过「仍使用 HTTP」放行一次；fetch/下载不受影响）。**解法已给出**：关闭「仅限 HTTPS」（设置→App→Safari→高级）后新标签页路径即可用；用户自行决定，尚未关闭。若用户最终选择不关，文件菜单的「分享」按钮需撤掉。
- 文件做不成图片那样的原生菜单：iOS 对链接/文件卡的长按菜单只会分享 URL 链接而非文件本体，与网页分享一样是 http 死路。
- **批量分享（多选栏）与 QQ 式预览：已讨论方案，待后续开工。**
...
- **用户 Safari 开着「仅限 HTTPS」**（疑似 iOS 26.6.1 引入）：拦截一切新开的 http 页面导航（App 首页靠手动放行一次；fetch 与下载不受影响）。影响：文件分享的「新标签页」方案被拦（报错「导览失败…仅限HTTPS」）。关闭路径：设置→App→Safari→高级→仅限 HTTPS。此设置决定文件分享能否启用，用户自决。
